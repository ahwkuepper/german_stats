...
def main():
  ...
  for filename in args:
      ...
      text = re.sub(r'[0-9]+,[0-9]+', r'', text_wo_perc)
      temp = re.search('([\w.-]+)\s+80.523.800', text)
      inhabitants = temp.group(1)
      temp = re.search('([\w.-]+)\s+2.399.409', text)
      students = temp.group(1)
      temp = re.search('([\w.-]+)\s+495.088', text)
      freshmen = temp.group(1)
      temp = re.search('([\w.-]+)\s+413.338', text)
      degrees = temp.group(1)
      temp = re.search('([\w.-]+)\s+353.690', text)
      employees = temp.group(1)
      temp = re.search('([\w.-]+)\s+43.862', text)
      professors = temp.group(1)
      temp = re.search('([\w.-]+)\s+204.644', text)
      foreign_students = temp.group(1)
      temp = re.search('([\w.-]+)\s+79.537', text)
      foreign_freshmen = temp.group(1)
      temp = re.search('([\w.-]+)\s+30.806', text)
      foreign_degrees = temp.group(1)
      temp = re.search('([\w.-]+)\s+35.345', text)
      foreign_employees = temp.group(1)
      temp = re.search('([\w.-]+)\s+2.778', text)
